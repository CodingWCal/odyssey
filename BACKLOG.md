...
### ODY-120 · New-trip wizard: "Add member" is easy to miss on mobile — S, sonnet (P1) — 🔴 MVP — ✅ DONE (2026-09-22)
> **Shipped:** `create()` now flushes a typed-but-unadded email into the invite list on submit (and on blur), so a forgotten tap on "Add" no longer silently drops the invite. The Add button also gets full-width, higher-contrast styling below 480px.
...
### ODY-121 · Date-picker inputs are smashed together on mobile — S, sonnet (P1) — 🔴 MVP — 🟡 PARTIAL, round 3 shipped 2026-09-23, awaiting re-verification
> **Shipped:** `.field-row`/`.wz-dates`/`.poll-form .field-row` already had proper grid gaps in code (predates this round's filing), but lacked `min-width: 0` on `.field` — a classic grid-overflow gap where a native date/time input's intrinsic width can push past its 1fr track regardless of the gap. Fixed, plus an explicit `@media (max-width: 400px)` single-column stack for all three date-row call sites so they never fight for space.
> **Round 2 (2026-09-23) — owner screenshot (filed as ODY-131) caught a real gap in the above.** The Add-event modal's Starts/Ends row was still overflowing the container edge on the owner's phone. Root cause: the 400px breakpoint only covers the narrowest phones — a **native iOS date/time input has its own OS-rendered minimum width that CSS can't shrink below**, so two side by side still overflow on ordinary phones above 400px (e.g. Pro Max-class devices at 428–430px CSS width, a large share of iPhones in use). `min-width: 0` on `.field` lets the *grid track* shrink, but can't make Safari's native control itself render narrower than its own chrome — the two are independent constraints, and only the first was fixed. **Fix: widened the stacking breakpoint from 400px to 768px** (`src/app/globals.css`), matching the app's own established `useIsMobile()` mobile/desktop split already used in four other places in `globals.css`. Every phone now stacks to one column, structurally eliminating the overflow (a full-width single field can't overflow itself, regardless of any browser's native-control minimum width); desktop (≥768px, ample room, no native-control risk) keeps the two-column layout. Covers all three call sites unchanged. **Verified by construction + owner screenshot, not by rendered browser** — this sandbox has Chromium only, and Chromium's native `<input type=time/date>` doesn't reproduce Mobile Safari's minimum-width quirk, so a sandbox screenshot here would prove nothing about the actual bug; the fix removes the side-by-side layout entirely on every mobile width rather than trying to out-guess a platform rendering minimum.
> **Round 3 (2026-09-23) — round 2's "structurally eliminates the overflow" claim was wrong; owner screenshot proved it.** Post-round-2, the owner's Starts/Ends boxes were *empty and overflowing off the right edge of the screen entirely* — worse than before, and on the exact same single-column layout that was supposed to fix it. Root cause: switching `.field-row` to a single **grid** column (`grid-template-columns: 1fr`) doesn't actually guarantee full-container width on real iOS Safari. A bare `1fr` track's minimum size is `auto`, and Safari resolves that `auto` from the native time/date control's own intrinsic content size rather than the container — `min-width: 0` on the grid *item* (`.field`, set in round 1) doesn't override the *track's* own sizing algorithm, so the track itself could still size to the control's preferred width and overflow. This is a genuine Safari-vs-Chromium rendering divergence in CSS Grid's intrinsic-sizing algorithm, not reproducible in this sandbox's Chromium-only environment (confirmed: built a pixel-faithful offline reproduction of the modal using the actual shipped/compiled CSS and measured box widths via `getBoundingClientRect` — Chromium showed all fields byte-identical at 350px both before and after this fix, i.e. Chromium never had the bug either round, so it could not have caught this). **Fix: stopped using CSS Grid for the stacked case.** `@media (max-width: 768px)` now sets `.field-row`/`.wz-dates`/`.poll-form .field-row` to `display: flex; flex-direction: column` instead of a 1-column grid. A flex child's width is computed from the container's box, not an intrinsic-content track calculation, so it structurally cannot blow out regardless of a native control's own rendering — this sidesteps the Grid quirk entirely rather than fighting it. **Still not owner-verified on the actual overflow fix** — the Chromium proof above only confirms no *new* regression, not that this resolves the real Safari bug; needs a fresh phone check after this deploys.
...
### ODY-122 · Explore: surface BOTH "Add to itinerary" and "Save to collections" on mobile — S, sonnet (P2) — 🔴 MVP — 🟡 PARTIAL (2026-09-22)
> **Shipped:** `.explore-card-actions` already `flex-wrap`s (predates this round's filing), so neither button was ever literally hidden — but the day picker could still crowd the row awkwardly on a wrapped line. Added an explicit `@media (max-width: 420px)` stack so the day picker and both buttons each get a clean full-width row. **Not owner-verified in a browser** — same sandbox limitation as ODY-121 (no auth/DB secrets here).
...
### ODY-124 · Itinerary: show the day's LOCATION in each day header (multi-destination clarity) — M, sonnet (P2) — 🔴 MVP — ✅ DONE (2026-09-22)
> **Shipped:** new `src/lib/deriveDayLocation.ts` (unit-tested) splits the trip's destination into candidate cities (handles the wizard's own `"City A & City B, Country"` format) and matches the first one found in that day's events' `location`; falls back to the full trip destination for single-destination trips or when nothing matches. Wired into `DayBlock.tsx`'s `day-title`, replacing the redundant weekday (the date line already shows it).
...
### ODY-125 · Dashboard: "N travelers" stat double-counts repeat collaborators and wraps ugly — S, haiku (P2) — 🔴 MVP — ✅ DONE (2026-09-22)
> **Shipped:** the dedup-by-`Set` logic already existed in `DashboardClient.tsx` (predates this round's filing) but deduped on `TripMember.id` — a *membership row's own id*, unique per trip — instead of the user's id, so it silently never collapsed repeat collaborators across trips (same bug as the raw sum, just hidden behind code that looked already fixed). Fixed `dashboard/page.tsx` to map `id: m.userId`. The mobile wrap CSS (`.stat-strip`) already wraps cleanly at ≤980px — no change needed there.
...
### ODY-126 · Trip notes sections: default-collapse noise, add-section toast, and move Packing List to its own mappable tab — M, sonnet (P2) — 🔴 MVP — ✅ DONE (2026-09-22, part 3 greenlit same day)
> **Shipped:** part 1 — `TripNotes.tsx`'s `collapsedSections` now defaults to every section collapsed except `default-reminders` (Important Reminders), instead of everything open. Part 2 — a one-time "Add a section" toast fires when a trip has never had a custom section saved (`note.sections.length === 0` server-side, before the client-only 3-starter-section fallback), dismissal persisted via a per-trip cookie (`ody-notes-hint-<tripId>`, `notes/actions.ts`), mirroring ODY-085's join-welcome pattern.
> **Part 3 — owner greenlit same day, shipped together with ODY-067 Stage B** (see that ticket for the per-event linking implementation). The itinerary's notes area no longer renders the "Packing List" section at all — `TripNotes.tsx` filters it out presentationally (`visibleSections`), and `defaultNoteSections()` no longer seeds it for new trips. **Deliberately non-destructive:** an existing trip's already-persisted "Packing List" content is left in `sections`/storage untouched (never deleted, never resaved-without-it) specifically so `packing/actions.ts`'s one-time `importLegacyPackingList` migration keeps working off the real data — silently dropping someone's never-yet-imported free-text packing notes on their next unrelated note edit would be exactly the "silent orphaning" ODY-067's own design notes warned against.
...
### ODY-127 · Auto-categorize event type from the title as you type — M, sonnet (P2) — 🔴 MVP — ✅ DONE (2026-09-22)
> **Shipped:** new `src/lib/categorizeEvent.ts` (pure, unit-tested keyword classifier, no dependencies) maps title keywords to a type per the ticket's exact groups. Wired into `AddEventModal.tsx`'s title `onChange` — only for a brand-new event and only until a `typeTouched` flag flips true (set when the user clicks a type chip), never on edit of an existing event.
...
### ODY-128 · Flights: layover / multi-leg support — M/L, sonnet (P2) — 🔴 MVP — ✅ DONE (2026-09-23, path a)
...
- **Shipped path (a):** `Event.layover String?` (nullable, additive), the optional "Layover" text field shown only when `type === "flight"` in `AddEventModal.tsx` (clears when switching away from flight), and displayed in `EventBlock.tsx`'s booking-details row when present. Wired through `createEventSchema`/`updateEventSchema` and both `createEvent`/`updateEvent` actions following the exact existing pattern for `confirmationCode`/`bookingUrl`/`checkIn`.
- **Deploy step completed (2026-09-23):** migration applied via Supabase's SQL Editor (this session's sandbox can't reach Postgres directly — no raw-TCP egress — so the SQL was generated offline via `prisma migrate diff` and handed to the owner to run). Merged to `main` immediately after, together with ODY-140 (same `Event` model, same migration).
- Full multi-leg (path b) stays unbuilt, per the ticket's own recommendation — only worth it if free-text (a) proves insufficient on a real trip.

### ODY-129 · Condense long addresses on mobile itinerary cards — S, sonnet (P2) — 🔴 MVP — ✅ DONE (2026-09-23)
> **Shipped — owner-confirmed format:** mobile shows the venue/street segment (text before the first comma) with a tap to reveal the full address inline; desktop unchanged; a location with no comma never gets a toggle. New pure `firstAddressSegment()` in `lib/utils.ts` (unit-tested), new `EventLocation` sub-component in `EventBlock.tsx` gated on `useIsMobile()`. Flight/transport routes go through `RouteLine`, which already solved this same overflow problem separately (ODY-096) — out of scope here.

### ODY-130 · Rename "Hotel" label to "Lodging" (covers Airbnb, hostels, etc.) — S, haiku (P2) — 🔴 MVP — ✅ DONE (2026-09-22)
> **Shipped:** added `TYPE_LABEL` to `Icons.tsx` (`hotel: "Lodging"`, everything else its capitalized self) and wired it into `TypeBadge.tsx`, `AddEventModal.tsx`'s and `CollectionsClient.tsx`'s type chips. Updated `mapTypes.ts`'s existing `TYPE_LABEL` too. Also fixed `MapClient.tsx`'s map-legend filter chips, which imported `TYPE_LABEL` but weren't using it — that spot was printing the raw lowercase type key independent of this ticket. Stored `type` value, CSS classes, icons, and color tokens unchanged.
...
### ODY-140 · Multi-night lodging: check-in/check-out dates + all-day banner — M, sonnet (P2) — ✅ DONE (2026-09-23)
> **In plain terms:** Booking a hotel for several nights shouldn't mean logging one event with a start/end *time* like a restaurant reservation. This gives lodging real check-in/check-out dates, and a multi-night stay now shows as a banner pinned to the top of every day it covers — "Check in" / "Staying" / "Check out" — like an all-day event on a calendar, instead of competing with the timed plan for that day.
- **Not a pre-filed ticket** — requested directly by the owner mid-session, immediately following ODY-130's "Hotel" → "Lodging" rename. Filed here retroactively for the record, same as ODY-126 part 3.
- **Scope decision made during build (owner confirmed):** the all-day banner repeats on **every day the stay spans** (check-in day through check-out day), not just the check-in day — matches a real calendar's all-day-event behavior, which is what the owner explicitly asked for.
- **Data model:** `Event.checkOutDate DateTime? @db.Date` (nullable, additive). `null` = ordinary same-day event, unchanged behavior for every type including single-night hotel stays. Set to a date *after* the event's own day = multi-night → pulled out of the day's normal timed list and rendered as an all-day banner instead. Check-in date is **not** a separate field — it's just the day the event already belongs to (`dayId`/`Day.date`), same as every other event type.
- **Label collision caught and avoided:** the existing ODY-086 booking-details field `checkIn` (free text, e.g. "After 3:00 PM") was already labeled "Check-in" for hotel type — adding a second, different "Check-in" label (the relabeled Starts/Ends time fields) would have been genuinely confusing. Relabeled the existing field to "Check-in note" to disambiguate; its underlying data/semantics are unchanged.
- **New pure logic (unit-tested):** `src/lib/lodging.ts` — `isMultiNightLodging()` and `lodgingPhaseForDay()`. `itinerary/page.tsx` computes, per day, which lodging events span it (one flatten + Map over the trip's already-fetched events — no extra query shape), and excludes those events from the day's normal timed list.
- **New component:** `src/components/itinerary/LodgingBanner.tsx`, rendered at the very top of `.day-body` (above day notes and the timed list) via a new `DayBlock` prop `allDayEvents`. Editing from any day the stay spans opens the same `AddEventModal` for the one underlying `Event` row — there's no duplication across days, so an edit updates the whole stay everywhere it shows.
- **Explicitly out of scope this round** (not asked for, would expand the change): map/budget/print/calendar-export do **not** get special multi-night-aware rendering — a lodging event still shows on the map/budget/print exactly as before (it's still one `Event` row with a `location`/`cost`), just not specially fanned out. Calendar export (`icsExport.ts`) still exports it as a single-day timed entry rather than a multi-day all-day ICS block. Revisit only if asked.
- **Acceptance:** a same-day/no-checkout hotel event behaves exactly as before (or any other type, unaffected). Setting a later checkout date on a hotel event removes it from its own day's timed list and shows it as a banner reading "Check in · [title]" on the check-in day, "Staying · [title]" on nights in between, and "Check out · [title]" on the last day; clicking any of them edits the one underlying event; viewers see the same banners read-only (no click). Server rejects a checkout date earlier than the check-in day regardless of what the client sends.
- **Files:** `prisma/schema.prisma`, `src/lib/validations/index.ts`, `src/app/trips/[tripId]/itinerary/actions.ts`, `src/lib/lodging.ts` (new) + test, `src/components/itinerary/AddEventModal.tsx`, `src/components/itinerary/DayBlock.tsx`, `src/components/itinerary/EventBlock.tsx`, `src/components/itinerary/LodgingBanner.tsx` (new), `src/app/trips/[tripId]/itinerary/page.tsx`, `src/types/index.ts`, `src/app/globals.css`.
- **Deploy step completed (2026-09-23):** migration run via Supabase's SQL Editor (this session's sandbox has no raw-TCP egress to Postgres at all — worked around it by diffing the schema offline with `prisma migrate diff --from-schema/--to-schema --script`, no DB connection needed, and handing the owner the exact SQL to paste). Covered all three pending columns in one shot: `Event.layover` (ODY-128), `Event.checkOutDate` (this ticket), and `ChecklistItem.eventId` (ODY-067 Stage B, which turned out to have never actually been applied either, despite being flagged first). Merged to `main` immediately after.

### ODY-141 · Manual override for a day's header location (multi-city trips) — S, sonnet (P2) — ✅ DONE (2026-09-23)
> **In plain terms:** ODY-124 made the day header show a place ("DAY 01 · Oahu") instead of repeating the weekday, guessed automatically from events already on that day. But on a multi-island/multi-city trip like Hawaii (Oahu, then Maui), the guess can't tell the two apart until you've already added an event with a matching location — annoying if you want to lay out the skeleton of the trip (which days are Oahu vs. Maui) before filling in events. This lets you just click the day's location and type the one you mean.
- **Not a pre-filed ticket** — requested directly by the owner mid-session; asked "is this already in the backlog?" first (it wasn't — checked before filing, per ODY-124's own scope, which only covered automatic derivation).
- **Zero schema change, zero migration risk:** `Day.label String?` already existed in `schema.prisma`, already live in production, and was already fully typed on `TripDay` — just never read or written by any UI. Repurposed as exactly this override. Merged to `main` immediately (ahead of ODY-128/140, which needed the migration above first).
- **Precedence:** `day.label` (manual override) wins when set, else `deriveDayLocation()`'s automatic guess (ODY-124), else the weekday fallback — exactly one extra `||` in the existing chain.
- **New action `updateDayLabel`** in `itinerary/actions.ts`, a direct mirror of the existing `updateDayNotes` (same `updateMany` scoped to `tripId`, same revalidate). New `DayLocationLabel.tsx`: click a small edit icon beside the day-title to turn it into an inline input, autosaving on blur (Enter blurs/saves, Escape reverts) — same interaction shape as `DayNotes.tsx`. Lives inside `DayBlock`'s already-clickable (collapse-toggle) header, so every click/keydown on the edit icon and the input itself stops propagation — otherwise typing a space, or even just opening the editor, would also toggle the day collapsed.
- **Acceptance:** clicking the edit icon turns the day location into a text input; typing "Maui" and saving shows "DAY 03 · Maui" even with no events yet; clearing it back to blank falls back to the automatic guess; viewers see a plain, non-editable heading; toggling day collapse/expand still works normally by clicking anywhere else in the header.
- **Files:** `src/app/trips/[tripId]/itinerary/actions.ts`, `src/components/itinerary/DayLocationLabel.tsx` (new), `src/components/itinerary/DayBlock.tsx`, `src/app/globals.css`.

### ODY-142 · Day notes: long text is invisible past line 1 — S, sonnet (P2) — ✅ DONE (2026-09-23)
> **In plain terms:** A day's note (`DayNotes.tsx`) was a `rows={1}` textarea with no auto-grow — anything past the first line scrolled inside an invisible 1-row box instead of being readable. A long "wake up early, grab coffee, walk to the old town, visit the cathedral…" plan was simply gone until you clicked in and scrolled around blind.
> **Not a pre-filed ticket** — requested directly by the owner mid-session, alongside ODY-143 below.
- **Fix:** `DayNotes.tsx` now shows a 2-line preview by default (`rows={2}`); a **"More details"** link appears only when the text actually overflows those 2 lines (measured via `scrollHeight` vs `clientHeight`, same technique `NoteSection.tsx`/`TripNotes.tsx` already use for auto-grow) and expands the box to its full auto-grown height. Focusing the textarea to edit also expands it, so typing past line 2 is never hidden below the fold; "Show less" collapses back. No schema change — `Day.notes` storage is untouched, this is display-only.
- **Files:** `src/components/itinerary/DayNotes.tsx`, `src/app/globals.css` (`.day-notes-content`, `.day-notes-more`).
- **Acceptance:** a short note (≤2 lines) shows fully with no "More details" link; a long note shows its first 2 lines plus a "More details" link that reveals the rest; editing (click/focus) always shows the full text while typing.

### ODY-143 · Flight/transport route addresses still fully spelled out on mobile — S, sonnet (P2) — ✅ DONE (2026-09-23)
> **In plain terms:** ODY-129 condensed long single-point addresses on mobile event cards (e.g. a restaurant's street address) to just the venue/street segment with a tap to expand. Flights and transport events don't use that single-point path though — they render through `RouteLine` (origin → destination), which ODY-129's own code comment incorrectly assumed "already handles this (ODY-096)." It didn't: ODY-096 only fixed *overflow* (stacking long addresses vertically instead of clipping), never *length* — so "John F. Kennedy International Airport → Narita International Airport" still rendered fully spelled out on a phone, exactly the crowding ODY-129 was meant to fix everywhere.
> **Not a pre-filed ticket** — requested directly by the owner mid-session, alongside ODY-142 above; found by re-reading `EventBlock.tsx`'s own `EventLocation` comment against the actual `RouteLine.tsx` code rather than trusting the comment's claim.
- **Fix:** moved the exact same condense-with-tap-to-expand pattern from `EventLocation` into `RouteLine.tsx` itself (a new internal `RoutePoint` component, reusing the existing `firstAddressSegment()` helper and `useIsMobile()` hook), applied independently to each endpoint (origin and destination each expand on their own tap). Since `RouteLine` is the one shared component behind both the itinerary event card *and* the map popup card (ODY-096's whole reason for extracting it), both get the fix automatically with no per-caller change. Desktop unchanged.
- **Files:** `src/components/shared/RouteLine.tsx`, `src/app/globals.css` (`.route-pt-toggle`), corrected the stale comment in `src/components/itinerary/EventBlock.tsx`.
- **Acceptance:** on mobile, a flight/transport event's From/To each show just their first address segment with a tap-to-expand toggle, matching ODY-129's single-point behavior; desktop and the map popup are unaffected in layout, and the map popup gets the same condensing.

### ODY-144 · Multi-leg flights: separate leg cards + overnight badge — M/L, sonnet (P2) — ✅ DONE (2026-09-24, merged after migration)
> **Deploy step completed (2026-09-24):** owner ran the SQL below in the Supabase SQL Editor and confirmed `legs | jsonb | YES` via `information_schema.columns`. Adds `Event.legs Json?` (nullable, additive — no data loss, no rewrite of existing rows). Generated offline via `prisma migrate diff` since this sandbox has no live DB connection (raw TCP to Supabase is blocked here) — same pattern as ODY-067/128/140:
> ```sql
> -- AlterTable
> ALTER TABLE "Event" ADD COLUMN     "legs" JSONB;
> ```
> **In plain terms:** A one-stop flight rendered fine, but a flight with a layover (BOS → LAX → HNL) just showed one line of free text ("1h 20m in Denver") — nothing like the separate departure/arrival cards an airline app shows per leg. This replaces that with a proper multi-leg model: the Add-event form gets a repeating From/To/Departs/Arrives/Flight-#/Operated-by block with an "Add another leg" button, and the itinerary card renders each leg as its own mini boarding-pass-style card connected by an auto-computed layover duration — plus a small "+1" badge on any flight (single- or multi-leg) that lands the next calendar day.
> **Data model:** `Event.legs: FlightLeg[] | null` (`src/lib/flightLegs.ts` has the shape) — flight-type only. `location`/`destLocation`/`startTime`/`endTime` are still the real top-level fields everything else in the app reads (map pins, day sort, budget); when `legs` is present the **server** derives those from the first/last leg, so nothing outside the flight display needed to change. A pre-existing single-leg flight (`layover` free text, no `legs` yet) is never auto-migrated — editing it just seeds leg 1 from its existing flat fields, and only writes `legs` once the user hits Save; `layover` itself is force-cleared only once a flight actually has 2+ legs, so an unedited old flight keeps showing its old text exactly as before.
> **Day-crossing, no date pickers:** legs only take times (Departs/Arrives), not dates — an overnight leg or overnight layover is inferred purely from clock-time comparison (`computeLegDayOffsets` in `lib/flightLegs.ts`, unit-tested): if a leg's arrival clock time is earlier than its departure, or the next leg's departure is earlier than the previous arrival, it's assumed to cross midnight. Caps at +1 day per hop. **Known limitation (corrected 2026-09-24 — the original "true for every real flight" claim was wrong):** times are each airport's *local* clock, so an **eastbound International Date Line crossing** gives a false "+1" — e.g. Tokyo 17:00 → LAX 10:00 lands the *same* calendar date, but 10:00 < 17:00 reads as next-day. Westbound transpacific (LAX 23:00 → Tokyo 05:00, really +2) under-counts to "+1". US domestic, Hawaii, transatlantic, and Europe/Asia hops are all correct. A real fix needs per-leg dates or airport timezones — scoped out; revisit only if an owner trip actually hits it. The badge correctly catches the case where a same-day-looking final leg still lands a day later because of an *overnight layover*, not just an overnight leg.
> **Scope decisions made without asking (flagged here, not guessed silently):** no per-leg map pins (top-level lat/lng only, unchanged map behavior) — legs still store each endpoint's picked coordinates for a future pass, just not wired to the map now; no seat/passenger data (screenshot reference was for structure — separate leg cards, route, times, flight #, operator — not a literal boarding-pass clone); the overnight indicator is an inline "+1" badge, not a top-of-day banner like multi-night lodging (owner's explicit choice — a banner is reserved for something spanning many days, not crossing one midnight).
> Owner-provided reference: an airline app's per-leg boarding-pass card (BOS→LAX, then LAX→HNL) — used for structure, not visual skin; rendered in the app's own editorial tokens (`--coral` flight color, `--paper-2`/`--rule` card, `--font-mono` times), no blue chrome or seat data.
- **Files:** `prisma/schema.prisma`, `src/lib/flightLegs.ts` (new, unit-tested), `src/lib/validations/index.ts`, `src/types/index.ts`, `src/app/trips/[tripId]/itinerary/actions.ts`, `src/app/trips/[tripId]/itinerary/page.tsx`, `src/components/itinerary/FlightLegsFields.tsx` (new, form), `src/components/itinerary/FlightLegsDisplay.tsx` (new, card), `src/components/itinerary/AddEventModal.tsx`, `src/components/itinerary/EventBlock.tsx`, `src/components/shared/RouteLine.tsx` (exported `RoutePoint` for reuse), `src/app/globals.css`.
- **Acceptance:** adding a flight can add 2+ legs via "Add another leg," each with its own from/to/departs/arrives/flight #/operator; the itinerary card shows each leg as its own card with an auto-computed layover between them; a single-leg flight is unchanged (still the plain RouteLine); any flight landing a later calendar day shows a "+1" badge next to its arrival time, including one that only crosses midnight during a layover; editing an old single-leg flight upgrades it into the new form without losing data; tsc/eslint/309 tests/build all clean. **Not yet owner-verified in a browser** — same sandbox limitation as everything else this session.
> **Pre-merge review fixes (2026-09-24), found re-reading the diff before shipping:** (1) **stale map pin** — when a leg's airport was typed rather than picked from the dropdown, the server's `derived?.lat ?? validated.lat ?? event.lat` chain fell through to the event's *old* coordinates, so retyping a flight from JFK to "Boston Logan" kept the pin at JFK. The derived (legs) path now uses only the leg's own coordinates and geocodes when they're missing. (2) **same bug, form side** — picking "JFK" then retyping the field kept JFK's coordinates on the leg; typing now clears a leg's coordinates so only a fresh pick sets them. (3) **copy/duplicate dropped legs** — `copyDayEvents` and `duplicateTrip` cloned every plan field except `legs`, so a copied BOS→LAX→HNL flight collapsed to a single BOS→HNL route with no layover. Both now copy `legs` (layover/booking fields still aren't copied — pre-existing behavior, unchanged).

### ODY-145 · Single-leg flights use the same leg-card UI as multi-leg — S, opus (P2) — ✅ DONE (2026-09-24)
> **In plain terms:** ODY-144 gave multi-leg flights an airline-app style card (flight #, big times, route, operator), but a plain one-leg flight still looks like every other event type — a single pin + "A → B" line. Every flight should get the card, one leg or many.
> **Not a pre-filed ticket** — requested directly by the owner after testing ODY-144 live ("it looks great… one leg flights look the same as other event types").
- **Two kinds of one-leg flight, both covered:** a flight saved through ODY-144's leg form already stores `legs` with one entry — it was only gated out by `legs.length > 1`. A **pre-ODY-144 flight** has no `legs` at all, just flat `location`/`destLocation`/`startTime`/`endTime`; new `flightLegsForDisplay()` (`lib/flightLegs.ts`) builds it a single leg from those fields — display-only, nothing written to the DB. A flight with no route to draw (no departure or no arrival) keeps the plain location line.
- **Found while building — false "+1" on older flights:** pre-ODY-144 flights can be missing a time, and the day-crossing math read a blank time as midnight, so a flight departing 10:10 PM with no arrival time would have shown "arrives next day." `minutesOf()` now returns `null` for a missing/unparseable time and a crossing is only inferred when both times are known; a layover with a missing time shows "Layover in X" with no duration; a missing time renders as "—" (matching the event-time rail). 9 new unit tests (red → green before implementing).
- **Found while building — uneven card widths on mobile:** on phones `.event-top` stacks into a column but keeps `align-items: flex-start`, so the content column shrinks to its content instead of spanning the card — a short route ("Honolulu, HI → Boston, MA") drew a visibly narrower bordered card than a long one. Fixed for flights (`.event-card.t-flight .event-main { align-self: stretch }`), measured 273px → 316px (full width) on the short-route card. Same root cause affects every event type — filed as ODY-146 rather than changed here, per the owner's "all other events are good."
- **Verified:** rendered the real `FlightLegsDisplay` component (server-rendered via `react-dom/server`, not hand-written markup) against the compiled production CSS in Chromium at 390px — single-leg form flight, older flight missing its arrival time ("—", no false "+1"), older red-eye ("+1"). Not yet owner-verified on a real phone.
- **Files:** `src/lib/flightLegs.ts`, `src/lib/__tests__/flightLegs.test.ts`, `src/components/itinerary/FlightLegsDisplay.tsx`, `src/components/itinerary/EventBlock.tsx`, `src/app/globals.css`. No schema change.
- **Acceptance:** every flight with a from and to renders as a leg card on the itinerary, one leg or many; a pre-ODY-144 flight gets the card without being edited; a missing time never produces a "+1" or a bogus layover duration; flight cards are full card width on mobile regardless of route length.

### ODY-148 · Collections: replace per-card day picker + button with a "+" that opens a day modal — S, opus (P2) — ✅ DONE (2026-09-24)
> **In plain terms:** Every saved place on the Collections tab carries its own full-width day dropdown and a big "Add to itinerary" button, so the list reads as a wall of controls instead of your saved places. Just show the places, with a small "+" on each; tapping it opens a picker to choose the day.
> **Not a pre-filed ticket** — requested directly by the owner with a phone screenshot of the Collections tab.
- **Shipped:** each saved place (`CollectionsClient.tsx`) now shows only its details plus a compact icon pair in the card's top-right — a peri "+" and the existing remove. "+" opens the shared `Modal` (a bottom sheet on phones) titled "Add to itinerary · Pick a day for {place}", listing every trip day as a one-tap row — the same `.copy-day-option` list the itinerary's "Copy this day's events to…" already uses, so there's no new picker pattern. Tapping a day adds the place (existing `addPlaceToItinerary`, unchanged) and closes with "Added "Jugem" to Day 03."; on failure the sheet stays open to retry. The place stays in the collection, as before. Viewers still see no actions. No server or schema change.
- **Removed:** the per-card `<select>` + "Add to itinerary" button, the `dayPick` per-card state, and the mobile rule that stacked every card into a column (only needed because the old 150px control column crushed the place name).
- **Verified in a real browser:** bundled the real `CollectionsClient` with stubbed server actions and drove it with Playwright at iPhone width (390px) — 13/13 checks: no select/button left on cards, a "+" per card, card height 117–136px (≈290px each in the owner's screenshot), "+" opens the picker naming the place with all days, tapping Day 03 sends `{dayId: d3, placeId}`, sheet closes with a success toast, place stays listed, failure keeps the sheet open with an error toast, viewers see no actions, zero runtime errors. Not yet owner-verified on the live app.
- **Not changed (possible follow-ups):** the full geocoded addresses still wrap 3–4 lines on a phone — the ODY-129 condense-with-tap-to-expand treatment could apply here too; the Explore tab still uses the inline day-picker + button pattern this ticket removed from Collections.
- **Files:** `src/components/collections/CollectionsClient.tsx`, `src/app/globals.css`.
- **Acceptance:** Collections shows saved places without per-card day controls; "+" opens a day picker and one tap adds the place to that day, on phone and desktop.

### ODY-147 · Move an event to a different day (drag-and-drop + Day picker) — M, opus (P2) — ✅ DONE (2026-09-24)
> **In plain terms:** Add an event to the wrong day and today the only fix is to delete it and re-enter it. You should be able to drag it onto the right day, or just pick the right day when editing it.
> **Not a pre-filed ticket** — requested directly by the owner. Owner picked "both" over drag-only or picker-only: drag across days on desktop, plus a **Day** dropdown in the Edit event form, which is the path that works on a phone (drag handles are hidden on mobile) and for keyboard/screen-reader users.
- **Drag (desktop):** every day used to have its own `DndContext`, so a card physically couldn't leave its day. New `ItineraryBoard` (client) wraps all days in **one** shared context; each whole day (header + body) is also a droppable, so an **empty or collapsed** day is a valid target. Each `DayBlock` handles its own side of a drop via `useDndMonitor` — reorder within it, or the leaving/arriving half of a move — and only the leaving day calls the server. Optimistic: the card jumps immediately; if the server rejects, a board-wide resync puts it back (the day's own server props didn't change, so its existing sync-on-new-props wouldn't fire by itself) with an error toast.
- **Why a `DragOverlay`:** `.day-body` clips overflow (for its collapse animation), so the card itself would vanish at its own day's edge. The lifted copy follows the pointer in a fixed overlay, shows "Move to Day NN" while over another day, and the target day gets a dashed peri outline; the source card fades in place.
- **Unchanged within a day:** collision logic (`lib/dayDnd.ts`, unit-tested) targets another day as a whole only when the pointer is over it; over its own day, in the gap between days, or with no pointer (keyboard) it's the same closest-same-day-event reorder as before. Keyboard navigation is scoped to the event's own day through a narrowed view of dnd-kit's droppable registry, so keyboard users don't get dragged into other days — they use the Day picker.
- **Day picker (edit form, every device):** `AddEventModal` gets a Day dropdown when editing (not on add — a new event goes on the day whose "Add event" you clicked); labelled "Check-in day" for lodging. Sent as `dayId` in `updateEvent`, which now verifies the day belongs to the event's own trip (same IDOR guard as ODY-052) and appends it after that day's last event.
- **Lodging moves the whole stay:** moving a hotel shifts its checkout by the same number of days — in the form (visible before you save; checkout `min` follows the new day) and in the drag action (only same-day hotels are draggable; multi-night stays are banners, moved via the picker). Without this, a same-day hotel dragged to a later day would check out before it checks in and fail with a generic error. The form's shift uses a new timezone-safe `shiftDateKeyBy()` — `parseDateString` builds *local* midnight, which is a day off for anyone east of UTC.
- **Server:** new `moveEventToDay(eventId, targetDayId)` (Zod-validated ids, editor+, trip-scoped). Deliberately *not* `updateEvent(id, { dayId })` — `updateEvent` treats its payload as the full edit form, so a partial one would clear location/times/notes it leaves out. The checkout-before-check-in rule is now one shared helper used by create, edit and move. Linked budget expenses don't reference the day, so moves leave the budget untouched. No schema change (`dayId` already exists).
- **Also fixed:** the "Events copied." toast in `DayBlock` rendered in the *error* style and announced assertively (`toast()` defaults to `"error"`); now `"success"` like the rest of the app. **Not fixed, same bug elsewhere:** `TripCard`'s "Trip duplicated." — one-word fix, left out of scope.
- **Verified end-to-end in a real browser:** bundled the real `ItineraryBoard` + `DayBlock` (+ `EventBlock`, `AddEventModal`, …) with Vite, server actions stubbed to record calls, and drove real mouse drags with Playwright in Chromium — 21/21 checks: drag onto another day's event, onto an empty day, target highlight + "Move to Day 02" label, server failure puts the card back with no phantom copy, success toast styled as success, same-day reorder still calls `reorderEvents` (not a move), edit-form Day picker sends `dayId` and shifts a hotel's checkout Oct 1 → Oct 3, no picker on the Add form, zero runtime errors. 14 new unit tests. **Not yet owner-verified** on the live app (no auth/DB in this sandbox).
- **Files:** `src/components/itinerary/ItineraryBoard.tsx` (new), `src/lib/dayDnd.ts` (new) + tests, `src/components/itinerary/DayBlock.tsx`, `EventBlock.tsx`, `LodgingBanner.tsx`, `AddEventModal.tsx`, `src/app/trips/[tripId]/itinerary/actions.ts` + `page.tsx`, `src/lib/dates.ts` + tests, `src/lib/validations/index.ts`, `src/types/index.ts`, `src/app/globals.css`.
- **Acceptance:** on desktop, dragging an event onto another day (including an empty or collapsed one) moves it there and it stays after reload; a failed move puts it back; reordering within a day works as before; on any device, editing an event and changing its Day moves it; moving a hotel keeps its number of nights.

### ODY-146 · Mobile event cards: content column doesn't span the card (edit/delete not pinned right) — S, opus (P3) — 🔴 OPEN, needs owner go-ahead
> **In plain terms:** On a phone, a short event like "Dinner" has its edit/delete buttons sitting partway across the card instead of at the right edge where they're meant to be.
- **Root cause (measured, 2026-09-24):** the mobile block in `globals.css` switches `.event-top` to `flex-direction: column` but it keeps the desktop `align-items: flex-start`, so `.event-main` shrinks to its content's width (`flex: 1` now controls height, not width). The `margin-left: auto` meant to pin `.event-actions` right only reaches the edge of that shrunk column — on a 390px screen a "Dinner" restaurant card's actions end at x=269 vs the card edge at x=353.
- **Same root cause was already hit twice and patched per-symptom:** the collapsed note preview (let it wrap instead of clip) and the location/cost row (stretched items). ODY-145 patched it a third time, scoped to flights.
- **Fix:** one line in the mobile block — `.event-main { align-self: stretch; }` — then drop ODY-145's flight-only rule. Visible change for every event type on mobile (edit/delete move to the right edge; checklists/notes/booking rows span the full card), which is why it's held for an owner OK rather than shipped alongside ODY-145.
- **Acceptance:** at 375–430px, every event type's edit/delete sit at the card's right edge and every row inside the card spans its full width; no card overflows the viewport.

### ODY-131 · Trip overview: reduce mobile spacing — S, haiku (P2) — 🟡 folded into ODY-121, not independently open
...
> **Not actually this ticket's scope.** The owner's screenshot showed the Add-event modal's Starts/Ends time-input row overflowing the container edge — not a spacing/whitespace issue in a "trip overview" area, but the same start/end-input overflow bug ODY-121 already targeted (round 2's fix was itself incomplete — see ODY-121 round 3). Tracked entirely under **ODY-121** rather than duplicated here; this entry has no separate remaining scope.
- **Acceptance:** none of its own — see ODY-121 for status.
...
> **Remaining — F10, owner/browser-capable-session only.** The automated **axe** sweep + contrast measurement on the 5 main routes needs a rendered, *authenticated* browser (Clerk session + live DB) to reach gated app routes (dashboard, trip itinerary, budget, etc.) — this sandbox has Chromium but no auth/DB access, so it can't be closed from here (same limitation as ODY-121/122 originally). tsc / eslint / 291 tests / build clean.
...
### ODY-067 · Packing checklist — trip-level + optional per-event — M→L, sonnet — ✅ STAGE A + B DONE (2026-09-22)
...
> **Stage B completed (2026-09-22)** — owner greenlit alongside ODY-126 part 3. Per-event packing per the design decision already recorded above ("usually personal, not a shared checkbox"): **event-scoped items are always personal** (`ownerId`: the creator), never group/shared — there's no assignee concept for them. Schema: `ChecklistItem.eventId String?` + `event Event? @relation(..., onDelete: Cascade)` (deliberately **Cascade, not SetNull** like `Expense.eventId` — a packing reminder tied to a deleted event has no reason to persist; satisfies "deleting an event doesn't orphan stray items" without any app-layer cleanup code, same as `Day`→`Event`). New `addEventChecklistItem` action (`packing/actions.ts`); `toggleChecklistItem`/`removeChecklistItem` reused unchanged (already generic via `visiblePackingWhere`), extended to also revalidate `/itinerary` when the item they touch is event-scoped. New compact `EventChecklist.tsx` renders inside `EventBlock.tsx` (below event notes), fed by a single extra query in `itinerary/page.tsx` (`groupPackingItemsByEvent`, unit-tested) — not a bolt-on per-event fetch. Packing page gets a new "By activity" rollup grouping personal event-scoped items by event (sorted by day/position), reusing the existing `PackingList` component; the flat "For me" list now only holds un-scoped personal items so it doesn't fill up with every hike's boots reminder.
> **Deploy step completed (2026-09-23).** Turned out this one had actually never been applied either, despite being flagged first — the itinerary/packing pages' defensive try/catch fallbacks (added 2026-09-22, see the incident note above) were quietly carrying it the whole time. Applied together with ODY-128's `layover` and ODY-140's `checkOutDate` in one Supabase SQL Editor run (this sandbox has no raw-TCP egress to Postgres — worked around by diffing the schema offline and handing the owner the exact SQL). The defensive fallbacks are still in place and harmless now that the column exists — no need to remove them.
