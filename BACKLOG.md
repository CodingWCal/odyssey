...
> **F1 + F11 fixed (2026-08-19), aesthetic untouched.** F1: the shared desktop `Modal` now moves focus in on open, traps Tab within the dialog, and returns focus to the trigger on close — one change covering every dialog, zero visual change. F11 (color-blindness): the map side-list (events + collections) now shows type as an **icon + label**, and pin tooltips append the type name, so type reads without hue while `TYPE_HEX` and numbered pins stay exactly as designed. Remaining: F2 focus rings, F3 card-menu keyboard, F4 reduced-motion, F6 packing input label, F7 assertive error toasts, F9 skip link/landmarks, F10 axe+contrast (needs a browser). tsc / eslint / 244 tests / build clean.
