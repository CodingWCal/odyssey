...
### ODY-126 · Trip notes sections: default-collapse noise, add-section toast, and move Packing List to its own mappable tab — M, sonnet (P2) — 🔴 MVP — 🟡 PARTIAL (2026-09-22, parts 1–2 shipped, part 3 deferred)
> **Shipped:** part 1 — `TripNotes.tsx`'s `collapsedSections` now defaults to every section collapsed except `default-reminders` (Important Reminders), instead of everything open. Part 2 — a one-time "Add a section" toast fires when a trip has never had a custom section saved (`note.sections.length === 0` server-side, before the client-only 3-starter-section fallback), dismissal persisted via a per-trip cookie (`ody-notes-hint-<tripId>`, `notes/actions.ts`), mirroring ODY-085's join-welcome pattern. **Part 3 (moving Packing List to its own tab with per-event linking) intentionally not built this round** — per the ticket's own fallback, it's schema-shaped new-feature work that folds into ODY-067 Stage B and needs an explicit greenlight against the standing "no new features" directive, not a same-session polish fix.
