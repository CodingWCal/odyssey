...
- ~~**Next up:** ODY-024~~ — ✅ done (2026-08-09): one cents-honest `formatMoney` + trip currency across all 5 money surfaces. **⚠️ needs `npx prisma db push` before deploy** (adds `Trip.currency`).
- **Next up:** the remaining money queue — **ODY-116** (personal "you owe" line, S) → **ODY-115** (partial settle-up, S) → **ODY-114** (adjustment split, S). All three are small and now that `formatMoney` exists they're cheaper. Or run **ODY-108**, the last remaining audit.
