...
### ODY-002 · Reconcile Day records when trip dates change — M, sonnet
...
### ODY-003 · Normalize Day dates to UTC (timezone drift) — M, sonnet
...
### ODY-004 · Validate budget & split inputs server-side — S, haiku
...
### ODY-005 · Wrap multi-step mutations in transactions — S, haiku
...
### ODY-006 · Clerk invitation revocation only scans page 1 — S, haiku
...
### ODY-010 · Proxy Nominatim through an internal API route — M, sonnet
...
### ODY-011 · Migrate inline styles to the design system — L, sonnet (can split per file)
...
### ODY-013 · Error UX for server actions (Zod throws are invisible) — M, sonnet
...
### ODY-014 · Add route-level loading states — S, haiku
...
### ODY-015 · Fix the 6 deferred react-hooks lint suppressions — M, sonnet
...
### ODY-016 · Test foundation + CI — L, sonnet
...
1. ODY-001, 002, 003 (correctness core — do first, sonnet)
2. ODY-004, 005, 006 (small hardening batch — one haiku session)
3. ODY-013 + 014 (error/loading UX pair)
4. ODY-010 (geocode proxy) → ODY-015 (lint) → ODY-016 (tests/CI)
5. ODY-011/012 (styling consolidation — after tests exist)
6. P2/P3 by taste.
