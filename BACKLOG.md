...
### ODY-103 · Fix ODY-101 regression: "Show less" / "Add event" clipped after expanding a busy day on mobile — S, haiku
...
- Fix in `DayBlock.tsx`'s collapse `useLayoutEffect`: when settling open, set `max-height: none` instead of a fixed `3000px` — no artificial ceiling once expanded, so `overflow: hidden` never clips real content.
- Preserve the smooth *collapsing* animation (`none → 0` doesn't transition) by explicitly setting `max-height` to the current `scrollHeight` px value first, then flipping to `0px` on the next frame (`requestAnimationFrame`) so the collapse always has a concrete pixel starting point to animate from, regardless of content height.
- Files: `src/components/itinerary/DayBlock.tsx`.
...
### ODY-102 · Trip notes card unusable on mobile: tiny scroll box, not collapsible — S, haiku
...
- Auto-grow: on mount and on every keystroke, size the `<textarea>` to its content (`el.style.height = "auto"; el.style.height = el.scrollHeight + "px"`) so `.notes-card` is always exactly as tall as the note — no internal scrollbar. Add `overflow-y: hidden` to `.notes-editor` (standard auto-grow pairing) so there's no scrollbar flash while resizing.
- Collapsible: add a chevron toggle in `.notes-head` (reuse `Icons.chevron`, rotate on collapse like `DayBlock`'s day-header chevron) that shows/hides the textarea; default **expanded** (no change to current default visibility). Simple conditional render — do not reuse `DayBlock`'s fixed-max-height collapse animation trick (see ODY-103, that exact pattern is what caused the clipping bug there).
- Guardrails: no inline styles, no hardcoded hex, no new deps; keep autosave-on-blur behavior unchanged; editorial aesthetic (periwinkle chevron/accent, matching `.icon-btn` sizing).
...
### ODY-104 · Shared notes sections (groceries, packing, etc.) under trip notes — M, sonnet (P3, follow-up to ODY-102)
> **In plain terms:** Beyond the one freeform pinned note, let a trip have named, collapsible mini-lists — "Groceries," "Packing," whatever the group needs — living under the main trip notes.
...
- Needs a data-shape decision: extend the existing `Note.content` JSON (`{ v:1, text, doc, sections?: [{ id, title, text }] }`) vs. a new lightweight model — prefer extending `Note.content` first (no schema/db-push needed) unless sections need independent ownership/permissions.
- UI: an "Add section" affordance under the pinned note; each section is its own small collapsible card (title + freeform text, same auto-grow textarea as ODY-102); reorder is a nice-to-have, not required for v1.
- Pairs with ODY-051 (canonical Note.content shape) and ODY-060 (Notes IA) — resolve those first or coordinate so this doesn't create a third notes-writing surface.
- Guardrails: editor+ to add/edit sections, viewers read-only (ODY-001); no new deps; editorial aesthetic; reuse ODY-102's auto-grow + collapse pattern per section (not `DayBlock`'s fixed-cap pattern).
- Acceptance: a trip can have multiple named, independently-collapsible note sections under the pinned trip notes; existing pinned note behavior (ODY-102) is unaffected.
