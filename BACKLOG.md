...
### ODY-144 · Multi-leg flights: separate leg cards + overnight badge — M/L, sonnet (P2) — 🟡 READY, held for schema migration (2026-09-23)
> ⚠️⚠️ **REQUIRED DEPLOY STEP before merging to `main`:** adds `Event.legs Json?` (nullable, additive — no data loss, no rewrite of existing rows). This sandbox has no live DB connection (raw TCP to Supabase is blocked here, established earlier this session), so the migration SQL below was generated offline via `prisma migrate diff` and needs to run by hand in the Supabase SQL Editor before this branch merges — same pattern as ODY-067/128/140:
> ```sql
> -- AlterTable
...
> **Day-crossing, no date pickers:** legs only take times (Departs/Arrives), not dates — an overnight leg or overnight layover is inferred purely from clock-time comparison (`computeLegDayOffsets` in `lib/flightLegs.ts`, unit-tested): if a leg's arrival clock time is earlier than its departure, or the next leg's departure is earlier than the previous arrival, it's assumed to cross midnight. Caps at +1 day per hop — true for every real commercial flight/layover. The badge correctly catches the case where a same-day-looking final leg still lands a day later because of an *overnight layover*, not just an overnight leg.
