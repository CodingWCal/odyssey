...
### ODY-121 · Date-picker inputs are smashed together on mobile — S, sonnet (P1) — 🔴 MVP — 🟡 PARTIAL (2026-09-22)
> **Shipped:** `.field-row`/`.wz-dates`/`.poll-form .field-row` already had proper grid gaps in code (predates this round's filing), but lacked `min-width: 0` on `.field` — a classic grid-overflow gap where a native date/time input's intrinsic width can push past its 1fr track regardless of the gap. Fixed, plus an explicit `@media (max-width: 400px)` single-column stack for all three date-row call sites so they never fight for space. **Not owner-verified in a browser** — this sandbox has no Clerk/Supabase secrets to run an authenticated dev server; someone with a live env should confirm at 375px.
...
- **⚠️⚠️ REQUIRED DEPLOY STEP, HIGHER STAKES THAN USUAL — run `npx prisma db push` before merging this to `main`, not after.** This is not the same risk class as ODY-067 Stage B's `ChecklistItem.eventId` (a niche, feature-scoped table). `Event` is the single most-queried model in the app — itinerary, map, budget, print, schedule, and calendar export all fetch full `Event` rows via broad `include`s with no `select` narrowing, so Prisma tries to select the new `layover` column on **every one of those routes** the moment this code runs against a database that doesn't have it yet. The itinerary-page crash from ODY-067 Stage B (2026-09-22 incident, see that ticket and the two defensive-fix commits right after it) happened with a narrower blast radius than this would have. **Sequence this correctly this time: `prisma db push` first, confirm it succeeded, only then merge `claude/backlog-qol-updates-jiurea` → `main`.** Left un-merged on the feature branch specifically so that ordering can't be skipped by accident.
...
- **⚠️⚠️ REQUIRED DEPLOY STEP, SAME HIGH-STAKES CLASS AS ODY-128 — run `npx prisma db push` before merging to `main`.** This adds a second new column (`checkOutDate`) to the same `Event` model as ODY-128's `layover` — see that ticket's entry for why `Event` is the highest-blast-radius model in the app to leave unmigrated. **Left un-merged on `claude/backlog-qol-updates-jiurea` alongside ODY-128, on purpose, until the migration is confirmed run** — do not merge either to `main` first without the other, since both touch the same model and should ship in the same `prisma db push`.
...
- **Zero schema change, zero migration risk:** `Day.label String?` already existed in `schema.prisma`, already live in production, and was already fully typed on `TripDay` — just never read or written by any UI. Repurposed as exactly this override. Unlike ODY-128/140, this is safe to merge to `main` immediately.
...
> **⚠️ REQUIRED DEPLOY STEP — run `npx prisma db push` before this lands in production.** Adds `ChecklistItem.eventId` (nullable, additive — no data migration) + the `Event`↔`ChecklistItem` relation. Same blast-radius shape as Stage A's warning above: this sandbox has no live database credentials, so the schema change is written and `prisma generate` has run (types/build are clean), but the migration has **not** been applied anywhere. Until it is, every `ChecklistItem` write/read in this diff (event-scoped add/toggle/remove, the itinerary page's per-event query, the packing page's rollup query) will fail against a database that doesn't have the column yet.
