...
- **Next up (2026-08-30):** genuinely-open hyper-polish. Shippable here (no browser needed): **ODY-109** remaining (bulk row / column / "free the whole range" marking + mobile sticky day column) · **ODY-118 F2** (`:focus-visible` rings on the seamless inline note editors) · **ODY-085** (post-invite "you're in" welcome). Needs a rendered browser — owner or a browser-capable session: **ODY-118 F10** + **ODY-022** axe/contrast sweep on the 5 main routes · **ODY-097** budget IA/mobile judgment. Owner content call, not code: **F01** (the fabricated "4,200 travelers" stat + invented "Maya R." testimonial — remove / replace / wait for real data).
