...
### ODY-118 · Accessibility audit → remediation — M, sonnet — 🟡 IN PROGRESS
> **F1, F4, F6, F7, F8, F11 fixed (2026-08-19), aesthetic untouched.** F1: shared desktop `Modal` now moves focus in on open, traps Tab, and returns focus to the trigger on close — one change covering every dialog. F11 (color-blindness): map side-list (events + collections) shows type as an **icon + label** and pin tooltips append the type name, so type reads without hue while `TYPE_HEX` + numbered pins stay as designed. F4: reduced-motion block stills the badge pulse + `Globe3D` idle spin. F6: packing add-input `aria-label`. F7: error toasts announce assertively (split live regions). F8: decorative globe `aria-hidden`. All behavior/opt-in only — no visual change for normal users. Remaining: F2 focus-visible rings, F3 card-menu keyboard, F9 skip link/landmarks, F10 axe+contrast (needs a browser). tsc / eslint / 244 tests / build clean.
